# Test 3D System Setup Script (PowerShell)
# Windows 환경에서 실행

New-Item -ItemType Directory -Force -Path test-3d-system | Out-Null

# 존재 시에만 이동
if (Test-Path "assets/js") {
    Get-ChildItem -File -Path assets/js -Filter "3d-*.js" -ErrorAction SilentlyContinue | Move-Item -Destination test-3d-system -Force -ErrorAction SilentlyContinue 
}

Get-ChildItem -File -Path . -Filter "*-demo.html" -ErrorAction SilentlyContinue | Move-Item -Destination test-3d-system -Force -ErrorAction SilentlyContinue
...
"@ | Set-Content -Encoding UTF8 test-3d-system/README.md

# 안내 HTML(없을 때만)
$demo = "test-3d-system/space-dolphin-demo.html"
if (!(Test-Path $demo)) {
    @"
<!doctype html>
<meta charset="utf-8">
...
"@ | Set-Content -Encoding UTF8 $demo
}

Write-Host "`n[OK] test-3d-system 준비 완료. 포트 4100으로 로컬 서버를 실행하세요." -ForegroundColor Green
